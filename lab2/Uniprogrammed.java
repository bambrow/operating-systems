...
public class Uniprogrammed extends SchedulingAlgorithm {
	...
	public static void run(List<Process> processList, boolean verbose) {
		...
		while (terminatedProcesses.size() < numProcesses) {
			...
			if (runningProcess == null && blockedProcesses.size() == 0 && !readyProcesses.isEmpty()) {
			...
			if (!blockedProcesses.isEmpty()) {
				IOTime++;
				blockedProcesses.peek().IOBurstTime--;
				blockedProcesses.peek().IOTime++;
			}
			...
			// handle blocked processes
			if (!blockedProcesses.isEmpty() && blockedProcesses.peek().IOBurstTime == 0) {
				runningProcess = blockedProcesses.poll();
				int assignedCPUBurstTime = RandomNumberReader.randomOS(runningProcess.B);
				runningProcess.CPUBurstTime = assignedCPUBurstTime;
			}
		}
